#!/usr/bin/env python
"""Description:
Setup script for CRISPResso -- Software pipeline for the analysis of 
CRISPR-Cas9 genome editing outcomes from deep sequencing data
@status:  beta
@version: $Revision$
@author:  Luca Pinello
...
# TO INSTALL CRISPRESSO DEPENDECIENS IN A CUSTOM LOCATION SET THE ENV
# VARIABLE: CRISPRESSO_DEPENDENCIES_FOLDER
...
def install_dependencies():
    ...
    if CURRENT_PLATFORM not in [
            'Linux', 'Darwin'] and platform.architecture() != '64bit':
        sys.stdout.write(
    ...
    if dependencies_already_installed:
        ...
        for shell_profile in shell_profiles:
            ...
            if not os.path.exists(os.path.join(home, shell_profile)) or not line_to_add in open(
                    os.path.join(home, shell_profile)).read():
