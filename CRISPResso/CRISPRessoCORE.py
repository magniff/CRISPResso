...
def check_library(library_name):
    try:
        ...
        error(
            'You need to install %s module to use CRISPResso!' %
            library_name)
        sys.exit(1)
...
nt_complement = dict({'A': 'T', 'C': 'G', 'G': 'C',
                      'T': 'A', 'N': 'N', '_': '_', '-': '-'})
...
def find_wrong_nt(sequence):
    return list(set(sequence.upper()).difference(
        set(['A', 'T', 'C', 'G', 'N'])))


def get_ids_reads_to_remove(
        fastq_filename, min_bp_quality=20, min_single_bp_quality=0):
    ids_to_remove = set()
...
def filter_pe_fastq_by_qual(fastq_r1, fastq_r2, output_filename_r1=None,
                            output_filename_r2=None, min_bp_quality=20, min_single_bp_quality=0):
    ...
    if not output_filename_r1:
        output_filename_r1 = fastq_r1.replace(
            '.fastq', '').replace(
            '.gz', '') + '_filtered.fastq.gz'

    if not output_filename_r2:
        output_filename_r2 = fastq_r2.replace(
            '.fastq', '').replace(
            '.gz', '') + '_filtered.fastq.gz'
...
def filter_se_fastq_by_qual(
    ...
    if not output_filename:
        output_filename = fastq_filename.replace(
            '.fastq', '').replace(
            '.gz', '') + '_filtered.fastq.gz'
...
def process_df_chunk(df_needle_alignment_chunk):
    ...
    for idx_row, row in df_needle_alignment_chunk.iterrows():
        ...
        if not args.ignore_substitutions:
            ...
            if substitution_positions:
                substitution_positions = list(
                    np.hstack(substitution_positions))
        ...
        if args.expected_hdr_amplicon_seq:
            ...
            # HDR
            if (row.score_diff < 0) & (row.score_repaired >=
                                       args.hdr_perfect_alignment_threshold):
        ...
        any_positions = np.unique(np.hstack(
            [deletion_positions_flat, insertion_positions, substitution_positions])).astype(int)
        ...
        if df_needle_alignment_chunk.ix[idx_row,
                                        'NHEJ'] and args.window_around_sgrna:
        ...
        if df_needle_alignment_chunk.ix[
                idx_row, 'NHEJ'] and args.hide_mutations_outside_window_NHEJ:
            effect_vector_mutation[substitution_positions] += 1
            effect_vector_deletion[deletion_positions_flat] += 1
            effect_vector_insertion[insertion_positions] += 1
        ...
        if not df_needle_alignment_chunk.ix[idx_row, 'UNMODIFIED']:
            ...
            df_needle_alignment_chunk.ix[
                idx_row, 'n_mutated'] = len(substitution_positions)
            df_needle_alignment_chunk.ix[
                idx_row, 'n_inserted'] = np.sum(insertion_sizes)
            df_needle_alignment_chunk.ix[
                idx_row, 'n_deleted'] = np.sum(deletion_sizes)
            ...
            if PERFORM_FRAMESHIFT_ANALYSIS:
                del_positions_to_append = sorted(
                    set(exon_positions).intersection(
                        set(deletion_positions_flat)))
                ...
                if set(splicing_positions).intersection(
                        substitution_positions):
    ...
    try:
        ...
        if args.guide_seq:
            ...
            for current_guide_seq in args.guide_seq.split(','):
                ...
                cut_points += [m.start() + offset_fw for m in re.finditer(current_guide_seq, args.amplicon_seq)] + [
                    m.start() + offset_rc for m in re.finditer(reverse_complement(current_guide_seq), args.amplicon_seq)]
                sgRNA_intervals += [(m.start(),
                                     m.start() + len(current_guide_seq) - 1) for m in re.finditer(current_guide_seq,
                                                                                                  args.amplicon_seq)] + [(m.start(),
                                                                                                                          m.start() + len(current_guide_seq) - 1) for m in re.finditer(reverse_complement(current_guide_seq),
                                                                                                                                                                                       args.amplicon_seq)]
        ...
        if args.expected_hdr_amplicon_seq:
            ...
            aligned_ref, aligned_exp = pairwise2.align.globalxx(
                args.amplicon_seq, args.expected_hdr_amplicon_seq)[0][:2]
            identity_ref_rep = sum([1.0 for a, b in zip(
                aligned_ref, aligned_exp) if a == b]) / len(aligned_ref) * 100
        ...
        if args.donor_seq:
            ...
            positions_core_donor_seq = [(m.start(), m.start() +
                                         len(args.donor_seq)) for m in re.finditer('(?=%s)' %
                                                                                   args.donor_seq, args.expected_hdr_amplicon_seq)]
        ...
        if args.coding_seq:
            ...
            for exon_seq in args.coding_seq.strip().upper().split(','):
                ...
                en_exon = st_exon + len(exon_seq)
                exon_intervals.append((st_exon, en_exon))
                exon_positions = exon_positions.union(
                    set(range(st_exon, en_exon)))

                # consider 2 base pairs before and after each exon
                splicing_positions += [max(0, st_exon -
                                           2), max(0, st_exon -
                                                   1), min(len_amplicon -
                                                           1, en_exon), min(len_amplicon -
                                                                            1, en_exon +
                                                                            1)]

            exon_positions = sorted(exon_positions)

            # protect from the wrong splitting of exons by the users to avoid
            # false splicing sites
            splicing_positions = set(
                splicing_positions).difference(exon_positions)
        ...
        # we don't touch the exons at all, the read can be still modified
        # tough..
        NON_MODIFIED_NON_FRAMESHIFT = 0
        ...
        get_name_from_fasta = lambda x: os.path.basename(
            x).replace('.fastq', '').replace('.gz', '')

        if not args.name:
            if args.fastq_r2 != '':
                database_id = '%s_%s' % (get_name_from_fasta(
                    args.fastq_r1), get_name_from_fasta(args.fastq_r2))
            else:
        ...
        if args.min_average_read_quality > 0:
            info(
                'Filtering reads with average bp quality < %d ...' %
        ...
        if args.fastq_r2 == '':  # single end reads
            ...
            if not args.trim_sequences:
                ...
                output_forward_paired_filename = _jp(
                    'output_forward_paired.fq.gz')
                output_forward_unpaired_filename = _jp(
                    'output_forward_unpaired.fq.gz')
                output_reverse_paired_filename = _jp(
                    'output_reverse_paired.fq.gz')
                output_reverse_unpaired_filename = _jp(
            ...
            if FLASH_STATUS:
                raise FlashException(
                    'Flash failed to run, please check the log file.')
        ...
        N_READS_AFTER_PREPROCESSING = get_n_reads_fastq(
        ...
        # write .fa file only for amplicon the rest we pipe trough awk on the
        # fly!
        ...
        if args.expected_hdr_amplicon_seq:
            database_repair_fasta_filename = _jp(
                '%s_database_repair.fa' % database_id)
            ...
            try:
                ...
                while line:
                    ...
                    if line:
                        ...
                        identity_seq = eval(line.strip().split(
                        ...
                        if just_score:
                            ...
                            needle_data.append(
                                [id_seq, identity_seq, aln_query_len, aln_ref_seq, aln_str, aln_query_seq])

                if just_score:
                    needle_infile.close()
                    return pd.DataFrame(needle_data, columns=[
                                        'ID', 'score_' + name]).set_index('ID')
                else:
                    needle_infile.close()
                    return pd.DataFrame(needle_data, columns=[
                                        'ID', 'score_' + name, 'length', 'ref_seq', 'align_str', 'align_seq']).set_index('ID')
            except:
        ...
        # If we have a donor sequence we just compare the fq in the two cases
        # and see which one alignes better
        if args.expected_hdr_amplicon_seq:
        ...
        if args.expected_hdr_amplicon_seq:
            ...
            df_database_and_repair[
                'score_diff'] = df_database_and_repair.score_ref - df_database_and_repair.score_repaired
            ...
            df_needle_alignment = parse_needle_output(
                needle_output_filename, 'ref')
        ...
        if sr_not_aligned.count():
            ...
            fasta_not_aligned_filename = _jp(
                'not_aligned_amplicon_forward.fa.gz')
            ...
            needle_output_rc_filename = _jp(
                'needle_output_rc_%s.txt.gz' % database_id)
            ...
            if args.expected_hdr_amplicon_seq:
                ...
                with open(database_repair_rc_fasta_filename, 'w+') as outfile:
                    outfile.write(
                        '>%s\n%s\n' %
                        (database_id, reverse_complement(
            ...
            # merge the flow rev
            if args.expected_hdr_amplicon_seq:
                df_database_rc = parse_needle_output(
                ...
                df_database_and_repair_rc = df_database_rc.join(
                    df_database_repair_rc)
                ...
                df_database_and_repair_rc[
                    'score_diff'] = df_database_and_repair_rc.score_ref - df_database_and_repair_rc.score_repaired
            ...
            # reverse complement and invert the align string so we have
            # everything in the positive strand
            df_needle_alignment_rc['ref_seq'] = df_needle_alignment_rc[
                'ref_seq'].apply(reverse_complement)
            df_needle_alignment_rc['align_seq'] = df_needle_alignment_rc[
                'align_seq'].apply(reverse_complement)
            df_needle_alignment_rc['align_str'] = df_needle_alignment_rc[
                'align_str'].apply(lambda x: x[::-1])

            # fix for duplicates when rc alignment
            df_needle_alignment_rc.index = map(
                lambda x: '_'.join([x, 'RC']), df_needle_alignment_rc.index)

            # append the RC reads to the aligned reads in the original
            # orientation
            df_needle_alignment = df_needle_alignment.append(
                df_needle_alignment_rc)

            del df_needle_alignment_rc

        # check for duplicates
        try:
            assert df_needle_alignment.shape[0] == df_needle_alignment.index.unique().shape[
                0]
        except:
        ...
        df_needle_alignment['UNMODIFIED'] = (
            df_needle_alignment.score_ref == 100)
        ...
        if 'N' in args.amplicon_seq:
            ...
            def ignore_N_in_alignment(row):
                row['align_str'] = ''.join(
                    [('|' if (row['ref_seq'][idx] == 'N') else c) for idx, c in enumerate(row['align_str'])])
        ...
        df_needle_alignment['ref_positions'] = df_needle_alignment[
            'ref_seq'].apply(compute_ref_positions)
        ...
        if args.n_processes > 1:
            info(
                '[CRISPResso quantification is running in parallel mode with %d processes]' % min(
            ...
            pool = mp.Pool(
                processes=min(
                    ...
                    args.n_processes))
            chunks_computed = []
            for result in pool.imap(process_df_chunk, np.array_split(
                    df_needle_alignment, min(df_needle_alignment.shape[0], args.n_processes))):
            ...
            df_needle_alignment, effect_vector_insertion,\
                ...
                NON_MODIFIED_NON_FRAMESHIFT, SPLICING_SITES_MODIFIED = process_df_chunk(
                    df_needle_alignment)
        ...
        df_alleles = df_needle_alignment.groupby(
            ['align_seq', 'NHEJ', 'UNMODIFIED', 'HDR', 'n_deleted', 'n_inserted', 'n_mutated']).size()
        ...
        df_alleles['%Reads'] = df_alleles['#Reads'] / \
            df_alleles['#Reads'].sum() * 100

        if np.sum(np.array(map(int, pd.__version__.split('.')))
                  * (100, 10, 1)) < 170:
        ...
        lgd = plt.legend(['No indel', 'Indel'], loc='center', bbox_to_anchor=(
            0.5, -0.22), ncol=1, fancybox=True, shadow=True)
        ...
        plt.savefig(
            _jp('1a.Indel_size_distribution_n_sequences.pdf'),
            bbox_inches='tight')
        if args.save_also_png:
            plt.savefig(
                _jp('1a.Indel_size_distribution_n_sequences.png'),
                bbox_inches='tight')

        plt.figure(figsize=(8.3, 8))
        plt.bar(0, hdensity[center_index] /
                (float(hdensity.sum())) *
                100.0, color='red', linewidth=0)
        plt.hold(True)
        barlist = plt.bar(hlengths, hdensity /
                          (float(hdensity.sum())) *
                          100.0, align='center', linewidth=0)
        barlist[center_index].set_color('r')
        plt.xlim([xmin, xmax])
        plt.title('Indel size distribution')
        ...
        plt.savefig(
            _jp('1b.Indel_size_distribution_percentage.pdf'),
            bbox_inches='tight')
        if args.save_also_png:
            plt.savefig(
                _jp('1b.Indel_size_distribution_percentage.png'),
                bbox_inches='tight')
        ...
        if args.expected_hdr_amplicon_seq:
            ...
            if cut_points or args.donor_seq:
                ...
                ax2.plot([0, len_amplicon], [0, 0], '-k',
                         lw=2, label='Amplicon sequence')
                plt.hold(True)

                if args.donor_seq:
                    ax2.plot(core_donor_seq_st_en, [
                             0, 0], '-', lw=10, c=(0, 1, 0, 0.5), label='Donor Sequence')
                ...
                for idx, sgRNA_int in enumerate(sgRNA_intervals):
                    if idx == 0:
                        ax2.plot([sgRNA_int[0], sgRNA_int[1]], [0, 0],
                                 lw=10, c=(0, 0, 0, 0.15), label='sgRNA')
                    else:
                        ax2.plot([sgRNA_int[0], sgRNA_int[1]], [0, 0],
                                 lw=10, c=(0, 0, 0, 0.15), label='_nolegend_')
            ...
            if cut_points:
                ...
                ax2.plot(cut_points, np.zeros(len(cut_points)), 'vr',
                         ms=12, label='Predicted Cas9 cleavage site/s')
                plt.legend(
                    bbox_to_anchor=(
                        0,
                        0,
        ...
        plt.xlabel('Size (bp)')
        plt.ylabel('Sequences % (no.)')
        lgd = plt.legend(['Non-insertion', 'Insertion'][::-
                                                        1], bbox_to_anchor=(.85, -
                                                                            0.22), ncol=1, fancybox=True, shadow=True)
        ...
        y_label_values = np.round(
            np.linspace(
                0, min(
                    N_TOTAL, max(
                        ax.get_yticks())), 6))  # np.arange(0,y_max,y_max/6.0)
        plt.yticks(
            y_label_values, [
                '%.1f%% (%d)' %
                (n_reads / N_TOTAL * 100, n_reads) for n_reads in y_label_values])

        ax = fig.add_subplot(1, 3, 2)
        ax.bar(-x_bins_del[:-1], y_values_del, align='center', linewidth=0)
        barlist = ax.bar(-x_bins_del[:-1],
                         y_values_del,
                         align='center',
                         linewidth=0)
        barlist[0].set_color('r')
        ...
        plt.xlabel('Size (bp)')
        plt.ylabel('Sequences % (no.)')
        lgd = plt.legend(['Non-deletion', 'Deletion'][::-
                                                      1], bbox_to_anchor=(.85, -
                                                                          0.22), ncol=1, fancybox=True, shadow=True)
        lgd.legendHandles[0].set_height(6)
        lgd.legendHandles[1].set_height(6)
        ...
        lgd = plt.legend(['Non-substitution', 'Substitution'][::-1],
                         bbox_to_anchor=(.89, -0.22), ncol=1, fancybox=True, shadow=True)
        lgd.legendHandles[0].set_height(6)
        lgd.legendHandles[1].set_height(6)
        plt.xlim(xmin=-1)
        y_label_values = np.round(
        ...
        plt.tight_layout()
        plt.savefig(
            _jp('3.Insertion_Deletion_Substitutions_size_hist.pdf'),
            bbox_inches='tight')
        if args.save_also_png:
            plt.savefig(
        ...
        if cut_points:
            ...
            for idx, cut_point in enumerate(cut_points):
                if idx == 0:
                    plt.plot([cut_point, cut_point], [0, y_max], '--k',
                             lw=2, label='Predicted cleavage position')
                else:
                    plt.plot([cut_point, cut_point], [0, y_max],
                             '--k', lw=2, label='_nolegend_')
        ...
        lgd = plt.legend(loc='center', bbox_to_anchor=(
            0.5, -0.23), ncol=1, fancybox=True, shadow=True)
        y_label_values = np.arange(0, y_max, y_max / 6.0)
        plt.yticks(
            y_label_values, [
                '%.1f%% (%d)' %
                (n_reads / float(N_TOTAL) * 100, n_reads) for n_reads in y_label_values])
        plt.xticks(np.arange(0, len_amplicon, max(
            3, (len_amplicon / 6) - (len_amplicon / 6) % 5)).astype(int))
        ...
        y_max = max(
            ...
            max(effect_vector_mutation)) * 1.2
        ...
        lgd = plt.legend(loc='center', bbox_to_anchor=(
            0.5, -0.28), ncol=1, fancybox=True, shadow=True)
        y_label_values = np.arange(0, y_max, y_max / 6.0)
        plt.yticks(y_label_values,
                   ['%.1f%% (%.1f%% , %d)' % (n_reads / float(N_TOTAL) * 100,
                                              n_reads /
                                              float(N_MODIFIED) * 100,
                                              n_reads) for n_reads in y_label_values])
        plt.xticks(np.arange(0, len_amplicon, max(
            3, (len_amplicon / 6) - (len_amplicon / 6) % 5)).astype(int))
        ...
        if args.expected_hdr_amplicon_seq:
            ...
            y_max = max(
                ...
                max(effect_vector_mutation_hdr)) * 1.2
            ...
            # MIXED
            plt.figure(figsize=(10, 10))
            plt.plot(effect_vector_insertion_mixed,
                     'r', lw=3, label='Insertions')
            plt.hold(True)
            plt.plot(effect_vector_deletion_mixed,
                     'm', lw=3, label='Deletions')
            plt.plot(
            ...
            y_max = max(
                ...
                max(effect_vector_mutation_mixed)) * 1.2
        ...
        if cut_points:
            ...
            for idx, cut_point in enumerate(cut_points):
                if idx == 0:
                    ax1.plot([cut_point, cut_point], [0, y_max], '--k',
                             lw=2, label='Predicted cleavage position')
                else:
                    ax1.plot([cut_point, cut_point], [0, y_max],
                             '--k', lw=2, label='_nolegend_')
        ...
        if cut_points:
            ...
            for idx, cut_point in enumerate(cut_points):
                if idx == 0:
                    ax2.plot([cut_point, cut_point], [0, y_max], '--k',
                             lw=2, label='Predicted cleavage position')
                else:
                    ax2.plot([cut_point, cut_point], [0, y_max],
                             '--k', lw=2, label='_nolegend_')
        ...
        if PERFORM_FRAMESHIFT_ANALYSIS:
            ...
            for idx, exon_interval in enumerate(exon_intervals):
                if idx == 0:
                    ax2.plot(exon_interval, [
                             0, 0], '-', lw=10, c=(0, 0, 1, 0.5), label='Coding sequence/s')
                else:
                    ax2.plot(exon_interval, [0, 0], '-', lw=10,
                             c=(0, 0, 1, 0.5), label='_nolegend_')

            if cut_points:
                ax2.plot(cut_points, np.zeros(len(cut_points)), 'vr',
                         ms=25, label='Predicted Cas9 cleavage site/s')
            ...
            x, y = map(np.array, zip(
                *[a for a in hist_frameshift.iteritems()]))
            ...
            ax1.set_xticklabels([str(idx) for idx in [
                                idx for idx in range(-30, 31) if idx % 3]], rotation='vertical')
            ...
            ax2.set_xticklabels([str(idx) for idx in [
                                idx for idx in range(-30, 31) if (idx % 3 == 0)]], rotation='vertical')
            ...
            patches, texts, autotexts = ax.pie([SPLICING_SITES_MODIFIED,
                                               ...
                                               explode=(0.0, 0),
                                               colors=[
                (0.89019608, 0.29019608, 0.2, 0.8), (0.99607843, 0.90980392, 0.78431373, 0.8)],
                autopct='%1.1f%%')
            proptease = fm.FontProperties()
            proptease.set_size('xx-large')
            plt.setp(autotexts, fontproperties=proptease)
            ...
            # no coding
            plt.figure(figsize=(10, 10))
            plt.plot(effect_vector_insertion_noncoding,
                     'r', lw=3, label='Insertions')
            plt.hold(True)
            plt.plot(effect_vector_deletion_noncoding,
                     'm', lw=3, label='Deletions')
            plt.plot(effect_vector_mutation_noncoding,
                     'g', lw=3, label='Substitutions')

            y_max = max(
                max(effect_vector_insertion_noncoding),
                max(effect_vector_deletion_noncoding),
                max(effect_vector_mutation_noncoding)) * 1.2
            ...
            if cut_points:
                ...
                for idx, cut_point in enumerate(cut_points):
                    ...
                    for idx, sgRNA_int in enumerate(sgRNA_intervals):
                        if idx == 0:
                            plt.plot([sgRNA_int[0], sgRNA_int[1]], [
                                     0, 0], lw=10, c=(0, 0, 0, 0.15), label='sgRNA')
                        else:
                            plt.plot([sgRNA_int[0], sgRNA_int[1]], [0, 0], lw=10, c=(
                                0, 0, 0, 0.15), label='_nolegend_')
        ...
        if not args.keep_intermediate:
            ...
            if sr_not_aligned.count():
                files_to_remove += [fasta_not_aligned_filename,
                                    database_rc_fasta_filename,
                                    needle_output_rc_filename]

                if args.expected_hdr_amplicon_seq:
                    files_to_remove += [database_repair_rc_fasta_filename,
        ...
        def save_vector_to_file(vector, name):
            np.savetxt(_jp('%s.txt' % name),
                       np.vstack([(np.arange(len(vector)) + 1),
                                  vector]).T,
                       fmt=['%d',
                            '%.18e'],
                       delimiter='\t',
        ...
        with open(_jp('Quantification_of_editing_frequency.txt'), 'w+') as outfile:
            outfile.write(
                ...
                + (
                    ...
                    (N_MODIFIED, np.sum(
                        df_needle_alignment.ix[
                            df_needle_alignment.NHEJ, 'n_inserted'] > 0), np.sum(
                        df_needle_alignment.ix[
                            df_needle_alignment.NHEJ, 'n_deleted'] > 0), np.sum(
                        df_needle_alignment.ix[
                            df_needle_alignment.NHEJ, 'n_mutated'] > 0)))
                + (
                    '\t- HDR:%d reads (%d reads with insertions, %d reads with deletions, %d reads with substitutions)\n' %
                    (N_REPAIRED, np.sum(
                        df_needle_alignment.ix[
                            df_needle_alignment.HDR, 'n_inserted'] > 0), np.sum(
                        df_needle_alignment.ix[
                            df_needle_alignment.HDR, 'n_deleted'] > 0), np.sum(
                        df_needle_alignment.ix[
                            df_needle_alignment.HDR, 'n_mutated'] > 0)))
                + (
                    '\t- Mixed HDR-NHEJ:%d reads (%d reads with insertions, %d reads with deletions, %d reads with substitutions)\n\n' %
                    (N_MIXED_HDR_NHEJ, np.sum(
                        df_needle_alignment.ix[
                            df_needle_alignment.MIXED, 'n_inserted'] > 0), np.sum(
                        df_needle_alignment.ix[
                            df_needle_alignment.MIXED, 'n_deleted'] > 0), np.sum(
                        df_needle_alignment.ix[
                            df_needle_alignment.MIXED, 'n_mutated'] > 0)))
                + ('Total Aligned:%d reads ' % N_TOTAL))

        # write alleles table
        df_alleles.to_csv(
            _jp('Alleles_frequency_table.txt'),
            sep='\t',
            header=True)
        ...
        if PERFORM_FRAMESHIFT_ANALYSIS:
            ...
            save_vector_to_file(
                effect_vector_mutation_noncoding,
                'effect_vector_substitution_noncoding')

        save_vector_to_file(
            effect_vector_insertion,
            'effect_vector_insertion_NHEJ')
        save_vector_to_file(
            effect_vector_deletion,
            'effect_vector_deletion_NHEJ')
        save_vector_to_file(
            effect_vector_mutation,
            'effect_vector_substitution_NHEJ')
        save_vector_to_file(effect_vector_combined, 'effect_vector_combined')

        save_vector_to_file(avg_vector_ins_all,
                            'position_dependent_vector_avg_insertion_size')
        save_vector_to_file(avg_vector_del_all,
                            'position_dependent_vector_avg_deletion_size')

        pd.DataFrame(np.vstack([hlengths, hdensity]).T, columns=[
                     'indel_size', 'fq']).to_csv(_jp('indel_histogram.txt'), index=None, sep='\t')
        pd.DataFrame(
            np.vstack(
                [
                    x_bins_ins[
            ...
            columns=[
                'ins_size',
                'fq']).to_csv(
            _jp('insertion_histogram.txt'),
            index=None,
            sep='\t')
        pd.DataFrame(np.vstack([-x_bins_del[:-1],
                                y_values_del]).T,
                     columns=['del_size',
                              'fq']).to_csv(_jp('deletion_histogram.txt'),
                                            index=None,
                                            sep='\t')
        pd.DataFrame(
            np.vstack(
            ...
            columns=[
                'sub_size',
                'fq']).to_csv(
            _jp('substitution_histogram.txt'),
            index=None,
            sep='\t')
        ...
        if args.expected_hdr_amplicon_seq:
            save_vector_to_file(
                effect_vector_insertion_mixed,
                'effect_vector_insertion_mixed_HDR_NHEJ')
            save_vector_to_file(
                effect_vector_deletion_mixed,
                'effect_vector_deletion_mixed_HDR_NHEJ')
            save_vector_to_file(
                effect_vector_mutation_mixed,
            ...
            save_vector_to_file(
                effect_vector_mutation_hdr,
                'effect_vector_substitution_HDR')
        ...
        if args.dump:
            ...
            np.savez(
                _jp('effect_vector_insertion_NHEJ'),
                effect_vector_insertion)
            np.savez(
                _jp('effect_vector_deletion_NHEJ'),
                effect_vector_deletion)
            np.savez(
                _jp('effect_vector_substitution_NHEJ'),
                effect_vector_mutation)

            np.savez(_jp('effect_vector_combined'), effect_vector_combined)

            np.savez(
                _jp('position_dependent_vector_avg_insertion_size'),
                avg_vector_ins_all)
            np.savez(
                _jp('position_dependent_vector_avg_deletion_size'),
                avg_vector_del_all)

            df_needle_alignment.to_pickle(
                _jp('processed_reads_dataframe.pickle'))
            ...
            if args.expected_hdr_amplicon_seq:
                ...
                np.savez(
                    _jp('effect_vector_insertion_HDR'),
                    effect_vector_insertion_hdr)
                np.savez(
                    _jp('effect_vector_deletion_HDR'),
                    effect_vector_deletion_hdr)
                np.savez(
                    _jp('effect_vector_substitution_HDR'),
                    effect_vector_mutation_hdr)
