...
def check_library(library_name):
    try:
        ...
        error(
            'You need to install %s module to use CRISPRessoCompare!' %
            library_name)
        sys.exit(1)
...
def check_output_folder(output_folder):
    ...
    if os.path.exists(quantification_file) and profile_file:
        ...
        raise OutputFolderIncompleteException(
            'The folder %s  is not a valid CRISPResso output folder.' %
            output_folder)
...
def check_hdr_mode(output_folder_1, output_folder_2):
    hdr_1 = os.path.exists(
        os.path.join(
            output_folder_1,
            'effect_vector_insertion_HDR.txt'))
...
def load_cut_points_sgRNA_intervals(output_folder):
    ...
    sgRNA_intervals_file = os.path.join(
        output_folder, 'sgRNA_intervals.pickle')
...
def main():
    ...
    try:
        ...
        get_name_from_folder = lambda x: os.path.basename(
            os.path.abspath(x)).replace('CRISPResso_on_', '')

        if not args.name:
            database_id = '%s_VS_%s' % (get_name_from_folder(
                args.crispresso_output_folder_1),
        ...
        # LOAD DATA
        N_UNMODIFIED_1, N_MODIFIED_1, N_REPAIRED_1, N_MIXED_HDR_NHEJ_1, N_TOTAL_1 = parse_quantification(
            quantification_file_1)
        N_UNMODIFIED_2, N_MODIFIED_2, N_REPAIRED_2, N_MIXED_HDR_NHEJ_2, N_TOTAL_2 = parse_quantification(
            quantification_file_2)
        ...
        means_sample_1 = np.array([N_UNMODIFIED_1,
                                   N_MODIFIED_1,
                                   N_REPAIRED_1,
                                   N_MIXED_HDR_NHEJ_1,
                                   ]) / N_TOTAL_1 * 100
        means_sample_2 = np.array([N_UNMODIFIED_2,
                                   N_MODIFIED_2,
                                   N_REPAIRED_2,
                                   N_MIXED_HDR_NHEJ_2]) / N_TOTAL_2 * 100
        ...
        if args.save_also_png:
            plt.savefig(
                _jp('1.Comparison_Efficiency.png'),
                bbox_inches='tight')
        ...
        lgd = plt.legend(loc='center', bbox_to_anchor=(
            0.5, -0.3), ncol=1, fancybox=True, shadow=False)
        ...
        if cut_points:
            for idx, cut_point in enumerate(cut_points):
                if idx == 0:
                    plt.plot([cut_point, cut_point], [
                             min(-1, y_min), max(1, y_max)], '--k', lw=2, label='Predicted cleavage position')
                else:
                    plt.plot([cut_point, cut_point], [min(-1, y_min),
                                                      max(1, y_max)], '--k', lw=2, label='_nolegend_')

            for idx, sgRNA_int in enumerate(sgRNA_intervals):
                if idx == 0:
                    plt.plot([sgRNA_int[0], sgRNA_int[1]], [
                             min(-1, y_min), min(-1, y_min)], lw=10, c=(0, 0, 0, 0.15), label='sgRNA')
                else:
                    plt.plot([sgRNA_int[0], sgRNA_int[1]], [
                             min(-1, y_min), min(-1, y_min)], lw=10, c=(0, 0, 0, 0.15), label='_nolegend_')

        lgd2 = plt.legend(loc='center', bbox_to_anchor=(
            0.5, -0.2), ncol=1, fancybox=True, shadow=False)
        plt.xticks(np.arange(0, len_amplicon, max(
            3, (len_amplicon / 6) - (len_amplicon / 6) % 5)).astype(int))
        plt.xlabel('Reference amplicon position (bp)')
        plt.ylabel('Sequences Difference %')
