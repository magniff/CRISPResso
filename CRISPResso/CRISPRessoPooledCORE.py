...
def check_library(library_name):
    try:
        ...
        error(
            'You need to install %s module to use CRISPRessoPooled!' %
            library_name)
        sys.exit(1)
...
def main():
    try:
        ...
        if args.gene_annotations:
            ...
            try:
                df_genes = pd.read_table(
                    args.gene_annotations, compression='gzip')
        ...
        if RUNNING_MODE == 'ONLY_AMPLICONS' or RUNNING_MODE == 'AMPLICONS_AND_GENOME':
            ...
            if not len(df_template.Amplicon_Sequence.unique()
                       ) == df_template.shape[0]:
            ...
            for idx, row in df_template.iterrows():
                ...
                if not pd.isnull(row.sgRNA):
                    ...
                    for current_guide_seq in row.sgRNA.strip().upper().split(','):
                        ...
                        offset_fw = args.cleavage_offset + \
                            len(current_guide_seq) - 1
        ...
        if RUNNING_MODE == 'ONLY_AMPLICONS':
            ...
            df_template[
                'Demultiplexed_fastq.gz_filename'] = fastq_gz_amplicon_filenames
            ...
            aligner_command = 'bowtie2 -x %s -p %s -k 1 --end-to-end -N 0 --np 0 -U %s 2>>%s | samtools view -bS - > %s' % (
                custom_index_filename, args.n_processes, processed_output_filename, log_filename, bam_filename_amplicons)
            ...
            s1 = r"samtools view -F 4 %s 2>>%s | grep -v ^'@'" % (
                bam_filename_amplicons, log_filename)
            ...
            for idx, row in df_template.iterrows():
                ...
                crispresso_cmd = 'CRISPResso -r1 %s -a %s -o %s --name %s' % (
                    row['Demultiplexed_fastq.gz_filename'], row['Amplicon_Sequence'], OUTPUT_DIRECTORY, idx)

                if n_reads_aligned_amplicons[
                        -1] > args.min_reads_to_use_region:
                    if row['sgRNA'] and not pd.isnull(row['sgRNA']):
                        crispresso_cmd += ' -g %s' % row['sgRNA']

                    if row['Expected_HDR'] and not pd.isnull(
                            row['Expected_HDR']):
            ...
            df_template['n_reads_aligned_%'] = df_template[
                'n_reads'] / float(N_READS_ALIGNED) * 100
        ...
        if RUNNING_MODE == 'AMPLICONS_AND_GENOME':
            ...
            # find the locations of the amplicons on the genome and their
            # strand and check if there are mutations in the reference genome
            additional_columns = []
            for idx, row in df_template.iterrows():
                fields_to_append = list(
                    np.take(
                        get_align_sequence(
                            row.Amplicon_Sequence, args.bowtie2_index).split('\t'), [
                            0, 1, 2, 3, 5]))
            ...
            df_template = df_template.join(
                pd.DataFrame(
                    ...
                    columns=[
                        ...
                        'bpend',
                        'strand',
                        'Reference_Sequence']).set_index('Name'))
            ...
            for idx, row in df_template.iterrows():
                if row.Amplicon_Sequence != row.Reference_Sequence and row.Amplicon_Sequence != reverse_complement(
        ...
        if RUNNING_MODE == 'ONLY_GENOME' or RUNNING_MODE == 'AMPLICONS_AND_GENOME':
            ...
            # check you have all the files for the genome and create a fa idx
            # for samtools
            ...
            if os.path.exists(uncompressed_reference):
                ...
                cmd_to_uncompress = 'bowtie2-inspect %s > %s 2>>%s' % (
                    args.bowtie2_index, uncompressed_reference, log_filename)
                sb.call(cmd_to_uncompress, shell=True)

                info('Indexing fasta file with samtools...')
                #!samtools faidx {uncompressed_reference}
                sb.call('samtools faidx %s 2>>%s ' %
                        (uncompressed_reference, log_filename), shell=True)
        ...
        if RUNNING_MODE == 'ONLY_GENOME' or RUNNING_MODE == 'AMPLICONS_AND_GENOME':
            ...
            aligner_command = 'bowtie2 -x %s -p %s -k 1 --end-to-end -N 0 --np 0 -U %s 2>>%s| samtools view -bS - > %s' % (
                args.bowtie2_index, args.n_processes, processed_output_filename, log_filename, bam_filename_genome)
            sb.call(aligner_command, shell=True)
        ...
        if RUNNING_MODE == 'AMPLICONS_AND_GENOME':
            ...
            for idx, row in df_template.iterrows():
                ...
                fastq_filename_region = os.path.join(
                    MAPPED_REGIONS, 'REGION_%s_%s_%s.fastq.gz' %
                ...
                if os.path.exists(fastq_filename_region):
                    ...
                    if N_READS >= args.min_reads_to_use_region:
                        ...
                        crispresso_cmd = 'CRISPResso -r1 %s -a %s -o %s --name %s' % (
                            fastq_filename_region, row['Amplicon_Sequence'], OUTPUT_DIRECTORY, idx)
            ...
            df_template[
                'Amplicon_Specific_fastq.gz_filename'] = fastq_region_filenames
            ...
            if args.gene_annotations:
                df_template = df_template.apply(
                    lambda row: find_overlapping_genes(
                        row, df_genes), axis=1)
            ...
            for region in files_to_match:
                coordinates.append(os.path.basename(region).replace('.fastq.gz', '').replace(
                    '.fastq', '').split('_')[1:4] + [region, get_n_reads_fastq(region)])
            ...
            df_regions['n_reads_aligned_%'] = df_regions[
                'n_reads'] / float(N_READS_ALIGNED) * 100

            df_regions['Reference_sequence'] = df_regions.apply(
                lambda row: get_region_from_fa(
                    row.chr_id, row.bpstart, row.bpend, uncompressed_reference), axis=1)
            ...
            if np.sum(np.array(map(int, pd.__version__.split('.')))
                ...
                df_regions.sort_values(
                    by='n_reads', ascending=False, inplace=True)
        ...
        if RUNNING_MODE == 'ONLY_GENOME':
            ...
            coordinates = []
            for region in glob.glob(os.path.join(
                    MAPPED_REGIONS, 'REGION*.fastq.gz')):
                coordinates.append(os.path.basename(region).replace('.fastq.gz', '').split('_')[
                                   1:4] + [region, get_n_reads_fastq(region)])
            ...
            df_regions.bpend = df_regions.bpend.astype(int)
            print df_regions
            df_regions['sequence'] = df_regions.apply(lambda row: get_region_from_fa(
                row.chr_id, row.bpstart, row.bpend, uncompressed_reference), axis=1)
            ...
            for idx, row in df_regions.iterrows():
                ...
                if row.n_reads > args.min_reads_to_use_region:
                    ...
                    crispresso_cmd = 'CRISPResso -r1 %s -a %s -o %s' % (
                        row.fastq_file, row.sequence, OUTPUT_DIRECTORY)
                    crispresso_cmd = propagate_options(
                        crispresso_cmd, crispresso_options, args)
                    info('Running CRISPResso:%s' % crispresso_cmd)
                    sb.call(crispresso_cmd, shell=True)
        ...
        for idx, row in df_final_data.iterrows():
            ...
            if quantification_file:
                N_UNMODIFIED, N_MODIFIED, N_REPAIRED, N_MIXED_HDR_NHEJ, N_TOTAL = parse_quantification(
                    quantification_file)
                quantification_summary.append([idx,
        ...
        if not args.keep_intermediate:
            ...
            if RUNNING_MODE == 'ONLY_AMPLICONS':
                files_to_remove += [bam_filename_amplicons,
                                    amplicon_fa_filename]
