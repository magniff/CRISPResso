...
def main():
    try:
        ...
        get_name_from_folder = lambda x: os.path.basename(
            os.path.abspath(x)).replace(
            'CRISPRessoPooled_on_',
            '').replace(
            'CRISPRessoWGS_on_',
            '')
        ...
        df_comp[
            '(%s-%s)_Unmodified%%' %
            (args.sample_1_name, args.sample_2_name)] = df_comp[
            'Unmodified%%_%s' %
            args.sample_1_name] - df_comp[
            'Unmodified%%_%s' %
            args.sample_2_name]
        df_comp['(%s-%s)_NHEJ%%' %
                (args.sample_1_name, args.sample_2_name)] = df_comp['NHEJ%%_%s' %
                                                                    args.sample_1_name] - df_comp['NHEJ%%_%s' %
                                                                                                  args.sample_2_name]
        df_comp['(%s-%s)_HDR%%' %
                (args.sample_1_name, args.sample_2_name)] = df_comp['HDR%%_%s' %
                                                                    args.sample_1_name] - df_comp['HDR%%_%s' %
                                                                                                  args.sample_2_name]
        df_comp[
            '(%s-%s)_Mixed_HDR-NHEJ%%' %
            (args.sample_1_name, args.sample_2_name)] = df_comp[
        ...
        # now run CRISPRessoCompare for the pairs for wich we have data in both
        # folders
