...
def check_library(library_name):
    try:
        ...
        error(
            'You need to install %s module to use CRISPRessoWGS!' %
            library_name)
        sys.exit(1)
    ...
    with gzip.open(out_fastq_filename, 'w+') as outfile:
        ...
        for line in output.split('\n'):
            if line:
                ...
                if bpstart in positions and bpend in positions:
      ...
      C\|     \      ||  |/ _ (_  |     C\|     \
        ...
        for idx, row in df_regions.iterrows():
            ...
            if row['sequence']:
                ...
                info(
                    'Trim reads and create a fastq.gz file in: %s' %
                    fastq_gz_filename)
                ...
                df_regions.ix[
                    idx, 'fastq.gz_file_trimmed_reads_in_region'] = fastq_gz_filename
            ...
            if row['n_reads'] >= args.min_reads_to_use_region:
                ...
                crispresso_cmd = 'CRISPResso -r1 %s -a %s -o %s --name %s' %\
                    (row['fastq.gz_file_trimmed_reads_in_region'],
                     row['sequence'], OUTPUT_DIRECTORY, idx)
